...
export default function RegisterStaffPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/identity/auth/register-waiter`, {
        method: 'POST',
      ...
      <div className="w-full max-w-md space-y-8 card shadow-soft-lg p-8 sm:p-10">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-text-muted">
            Sau khi đăng ký, chủ cửa hàng cần thêm bạn vào danh sách nhân viên trong Cài đặt. Bạn chưa có menu riêng.
          </p>
        </div>
        <form className="mt-8 space-y-6" onSubmit={handleSubmit} method="post" action="#">
          ...
          {error && (
            <div className="rounded-md bg-red-50 border border-red-200 p-4">
              <div className="text-sm text-red-800 whitespace-pre-line font-medium">{error}</div>
            </div>
          )}
          ...
          <div className="text-center space-y-2">
            ...
            <Link href="/auth/register" className="text-sm text-text-muted hover:text-text block">
